...
impl LocalAIClient {
    ...
    pub fn from_context(ctx: &AppContext) -> Option<Self> {
        ...
        let provider_type = match (&api_keys.openai, &api_keys.anthropic, &api_keys.open_router) {
            (_, Some(_), _) | (_, _, Some(_)) => ProviderType::OpenAI,
            (Some(_), _, _) => ProviderType::OpenAI,
            _ => return None,
        };
